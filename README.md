...
## 6. Використання змінних оточення
   ...
   3. Рівні встановлення змінної:
      ...
      - В сесії (тимчасово)
        ...
        Після закриття терміналу побачимо, що змінна знову undefined.
   ...
   4. Дослідимо пріоритетність

      В .env файлі вже записано значення змінної.
      Встановимо тимчасову змінну середовища MY_VAR зі значенням "task 6 new". Побачимо, що після повторного звернення до змінної, виводиться значення "task 6 new". Отже значення, задане в поточній сесії терміналу, працює лише в межах цієї сесії терміналу, і перекриє .env або системне значення, якщо ті присутні.

      ![image](https://github.com/user-attachments/assets/2261bbbc-49cf-4d3a-bae4-cb2203353daf)

      Запустимо на рівні ОС та побачимо, що виводиться значення, задане на рівні ОС. Отже встановлення змінної безпосередньо перед запуском скрипта має найвищий пріоритет і перекриває всі інші значення.

      ![image](https://github.com/user-attachments/assets/1691bb73-00e6-4229-b41d-051c91dafc06)


   Пріоритетність від вищого до нижчого:
   
   1. Пряме значення при запуску скрипта.
   2. Термінал/сесія
   3. .env
   4. Значення у системі (наприклад, через панель змінних середовища в Windows)
